...
class CellRegPy:
    ...
    def _register_fov(self,
        ...
        # ------------------------------------------------------------
        # ------------------------------------------------------------
        # Stage-6 style: "Centroid-primary + spatial-correlation floor veto"
        # Centroid model drives clustering; spatial correlation only vetoes links.
        # Mirrors validate_alignment_script Option 3 (Centroid-primary with floor filter).
        # Enabled via cfg.apply_spatial_floor_filter (bool) and cfg.spatial_corr_floor (float).
        # ------------------------------------------------------------
        apply_floor = bool(getattr(cfg, "apply_spatial_floor_filter", False))
        spatial_corr_floor = float(getattr(cfg, "spatial_corr_floor", 0.5))

        # IMPORTANT: we compute spatial correlations directly on the aligned footprints
        # (like validate_alignment_script) so the veto is evaluated for ALL within-cluster
        # pairs, not only those present in neighbor lists.
        if apply_floor and str(model_used).strip().lower().startswith("centroid"):
            filtered_map = cell_to_index_map.copy()
            n_vetoed = 0
            n_sessions_local = int(filtered_map.shape[1])

            for r in range(filtered_map.shape[0]):
                row = filtered_map[r, :]
                present = [s for s in range(n_sessions_local) if row[s] > 0]
                if len(present) < 2:
                    continue
                ...
                for a_i in range(len(present)):
                    if veto:
                        break
                    si = present[a_i]
                    ci0 = int(row[si]) - 1  # convert to 0-based ROI index
                    for b_i in range(a_i + 1, len(present)):
                        sj = present[b_i]
                        cj0 = int(row[sj]) - 1

                        try:
                            fp_i = aligned_fps[si][ci0]
                            fp_j = aligned_fps[sj][cj0]
                ...
                if veto:
                    # Dissolve this multi-session cluster into singletons:
                    # keep only the first present session, remove the rest.
                    for s in present[1:]:
                        filtered_map[r, s] = 0
                    n_vetoed += 1

            cell_to_index_map = filtered_map
            print(f"  [Centroid-primary spatial floor] vetoed {n_vetoed} clusters (floor={spatial_corr_floor})")
...
# ============================================================================ #
#                        CONVENIENCE PIPELINE ENTRYPOINT                       #
# ============================================================================ #

def run_pipeline(mouse_folder: Union[str, Path],
                 cfg: Optional['CellRegConfig'] = None,
                 spatial_corr_floor: float = 0.5,
                 apply_spatial_floor_filter: bool = True,
                 save_csv: bool = True,
                 save_wide_csv: bool = True) -> Tuple[pd.DataFrame, Dict]:
    """
    Run the full CellRegPy pipeline for a single mouse folder path.

    This is a convenience wrapper around:
        CellRegPy(cfg).run([mouse_folder])

    Defaults are chosen to match your Stage-6 "centroid-primary + spatial floor veto":
      - centroid distance model drives clustering (cfg.model_type = "Centroid distance")
      - spatial correlation is only used as a post-hoc veto (cfg.apply_spatial_floor_filter=True)

    Outputs:
      - Returns (mouse_table, mouse_data)
      - Optionally writes:
          <mouse_folder>/1_CellReg/mouse_table.csv
          <mouse_folder>/1_CellReg/mouse_table_wide.csv
    """
    mouse_folder = Path(mouse_folder)

    cfg = cfg or CellRegConfig()

    # Default to centroid-primary unless user explicitly set otherwise
    if str(getattr(cfg, "model_type", "auto")).strip().lower() in ("", "auto", "none"):
        cfg.model_type = "Centroid distance"

    # Enable Stage-6 spatial floor veto (used inside _register_fov)
    cfg.apply_spatial_floor_filter = bool(apply_spatial_floor_filter)
    ...
    if save_csv:
        ...
        try:
            ...
            print(f"[run_pipeline] Warning: could not write CSV outputs: {e}")
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(
        description="Run the full CellRegPy pipeline for a single mouse folder."
    )
    ap.add_argument("mouse_folder", type=str, help="Mouse folder containing session subfolders")
    ap.add_argument("--spatial_corr_floor", type=float, default=0.5,
                    help="Spatial correlation veto floor (default: 0.5)")
    ap.add_argument("--no_spatial_floor_filter", action="store_true",
                    help="Disable the centroid-primary spatial floor veto")
    ap.add_argument("--figures_visibility", choices=["on", "off"], default="off",
                    help="Show figures during run (default: off)")

    # Common knobs (optional)
    ap.add_argument("--microns_per_pixel", type=float, default=None)
    ap.add_argument("--maximal_distance", type=float, default=None)
    ap.add_argument("--p_same_threshold", type=float, default=None)

    args = ap.parse_args()

    cfg = CellRegConfig()
    cfg.figures_visibility = args.figures_visibility

    # Defaults for your preferred final approach:
    cfg.registration_approach = "Probabilistic"
    cfg.model_type = "Centroid distance"

    if args.microns_per_pixel is not None:
        cfg.microns_per_pixel = float(args.microns_per_pixel)
    if args.maximal_distance is not None:
        cfg.maximal_distance = float(args.maximal_distance)
    if args.p_same_threshold is not None:
        cfg.p_same_threshold = float(args.p_same_threshold)

    run_pipeline(
        args.mouse_folder,
        cfg=cfg,
        spatial_corr_floor=args.spatial_corr_floor,
        apply_spatial_floor_filter=(not args.no_spatial_floor_filter),
        save_csv=True,
        save_wide_csv=True,
    )