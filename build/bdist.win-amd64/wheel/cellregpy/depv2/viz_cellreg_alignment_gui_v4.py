\
"""
viz_cellreg_alignment_gui_v4.py

CellRegPy validation GUI (Tkinter + Matplotlib).

Fixes vs v3:
1) Robust sibling ordering when UnixTime has None/NaN (no more argsort TypeError).
2) Adds an "anatomical / CellReg-aligned" view:
   - When you click an ROI, the pop-up now includes:
     a) Native view (each session in its own native coords)
     b) Registered overlay view (ALL session ROIs warped into the CellReg reference frame)

How registered overlay works
- Loads per-FOV registration results from:
    <mouse_folder>/1_CellReg/FOV*/Results/registration_results.npy
  which includes mean_image_alignment.translations (dx, dy, rot_deg) and reference_session_index.
- Uses those dx/dy/rot values to warp ROI polygon points into reference coords
  (translation + rotation about origin, matching how warp was applied during registration).
- Overlays all warped ROIs on the reference session background.

Important naming note
- "C" in the trace dropdown means an optional calcium trace array in Fall.mat, NOT C/C++.

Run:
  python viz_cellreg_alignment_gui_v4.py
  (then File -> Open Mouse Folder...)
or:
  python viz_cellreg_alignment_gui_v4.py --mouse-folder "C:\\path\\to\\mouse_folder"

Interactive/Notebook:
  from viz_cellreg_alignment_gui_v4 import launch
...
def _safe_sort_rows_by_unix_time(df: pd.DataFrame, rows: np.ndarray) -> np.ndarray:
    """
    Sort row indices by UnixTime but robust to None/NaN.
    If UnixTime is missing or all NaN, preserve original order.
    """
    ...
    order = np.argsort(np.where(ser.to_numpy(dtype=float, copy=False) >= 0, ser.to_numpy(dtype=float, copy=False), ser.to_numpy(dtype=float, copy=False)))
...
def _apply_dxdyrot_to_points(pts_xy: np.ndarray, dx: float, dy: float, rot_deg: float) -> np.ndarray:
    """
    Apply a 2D rigid transform (rotation about origin + translation) to points in (x,y).
    Matches how alignment_translations are extracted from tform.params in CellRegPy.
    """
...
def _guess_session_label_from_cellreg_path(p: str, known_sessions: List[str]) -> str:
    """
    registration_results.npy stores session_paths often pointing to .../suite2p/plane0/CellReg.mat
    We try to find a parent folder name that matches one of the known session labels.
    """
    try:
        P = Path(p)
        parents = [pp.name for pp in P.parents]
        for name in parents:
            if name in known_sessions:
                return name
    except Exception:
        pass
    # fallback: basename sans extension
    try:
        return Path(p).stem
    except Exception:
        return str(p)
...
class CellRegViewer(tk.Tk):
    def __init__(self, mouse_table: Optional[pd.DataFrame] = None, mouse_folder: Optional[Path] = None):
        super().__init__()
        ...
        self.mouse_folder: Optional[Path] = mouse_folder
        self.fov_reg: Dict[int, Dict[str, Any]] = {}  # fovID -> registration_results dict
        ...
        if self.mouse_folder is not None:
            self._load_fov_registration(self.mouse_folder)
...
4) Popups:
   - Native footprints across sessions
   - Registered overlay (all sessions in reference frame)
...
"""
    ...
    def _open_mouse_folder(self):
        ...
        try:
            ...
            self._load_fov_registration(folder_p)
            self.set_table(t)
    ...
    def _open_mouse_table(self):
        ...
        try:
            ...
            # attempt to infer mouse_folder from table location
            self.set_table(t)
        except Exception as e:
            messagebox.showerror("Failed to load mouse table", str(e))

    def _load_fov_registration(self, mouse_folder: Path):
        """
        Load per-FOV registration_results.npy into memory for anatomical overlay.
        """
        self.fov_reg.clear()
        try:
    ...
    def set_table(self, mouse_table: pd.DataFrame):
        ...
        t = mouse_table.copy()
        # stable sort by UnixTime when available; NaNs/None go last
        if "UnixTime" in t.columns:
            t["UnixTime_num"] = pd.to_numeric(t["UnixTime"], errors="coerce")
            t = t.sort_values(["UnixTime_num", "Session"], kind="mergesort").drop(columns=["UnixTime_num"], errors="ignore")
    ...
    def _draw_session_image_and_rois(self):
        ...
        # aspect='auto' reduces huge whitespace on wide windows
    ...
    def _select_row(self, row_idx: int):
        ...
        # Use first sibling fovID for registration overlay lookup
        fov_id = None
        if "fovID" in self.t.columns:
            try:
                fov_id = int(pd.to_numeric(self.t.loc[siblings, "fovID"], errors="coerce").mode().iloc[0])
            except Exception:
                fov_id = None

        self._highlight_selected(row_idx)
        self._plot_traces_for_rows(siblings)
        self._show_footprints_window_native(siblings, cid)
        self._show_footprints_window_registered_overlay(siblings, cid, fov_id)
        self.canvas.draw_idle()
    ...
    def _show_footprints_window_native(self, rows: np.ndarray, cid: int):
        """Native view: each session shown in its own coordinates."""
        assert self.t is not None
        n = len(rows)
        ...
        fig.suptitle(f"Native footprints | CellRegID {cid}", fontsize=12, fontweight="bold")
        fig.tight_layout(rect=[0, 0, 1, 0.95])
        plt.show(block=False)

    def _show_footprints_window_registered_overlay(self, rows: np.ndarray, cid: int, fov_id: Optional[int]):
        """
        Anatomical view: warp all session polygons into the reference coordinate system
        using registration_results.npy mean_image_alignment.translations.
        """
        assert self.t is not None

        if fov_id is None or fov_id not in self.fov_reg:
            # Don't spam modal popups; show a message in the status bar instead.
            self.status.configure(text=f"Selected cellRegID={cid} (no FOV registration found for overlay).")
            return

        reg = self.fov_reg[fov_id]
        ...
        sess_paths = reg.get("session_paths", None) or reg.get("sessions_list", None) or []
        ...
        if trans is None or trans.ndim != 2 or trans.shape[0] < 3:
            self.status.configure(text=f"Selected cellRegID={cid} (overlay unavailable: missing translations).")
            return

        # Build mapping from session label -> column index in translations
        known = self.sessions
        label_to_col: Dict[str, int] = {}
        for i, p in enumerate(sess_paths):
            lab = _guess_session_label_from_cellreg_path(str(p), known)
            if lab not in label_to_col:
                label_to_col[lab] = i

        # Identify reference session label
        ref_label = None
        if 0 <= ref_idx < len(sess_paths):
            ref_label = _guess_session_label_from_cellreg_path(str(sess_paths[ref_idx]), known)

        # reference background
        # Prefer reference session in reg; otherwise use first row's session
        ref_row = rows[0]
        if ref_label is not None:
            try:
                # pick sibling row matching ref_label
        ...
        bg_ref = np.asarray(ref_sd.max_proj if ref_sd.max_proj is not None else ref_sd.mean_img, dtype=float)

        # Create overlay figure
        fig, ax = plt.subplots(1, 1, figsize=(7.5, 7.5))
        ax.imshow(bg_ref, cmap="gray")
        ax.set_title(f"Registered overlay (FOV{fov_id}) | CellRegID {cid}\nReference: {ref_label or ref_sess}", fontsize=10)
        ax.axis("off")
        ...
        for j, r in enumerate(rows):
            ...
            poly = _roi_polygon_from_stat(sd.stat[roi_id]) if (0 <= roi_id < len(sd.stat)) else None
            if poly is None or poly.size == 0:
                continue

            col = label_to_col.get(sess, None)
            if col is None or col >= trans.shape[1]:
                # can't transform; draw in native (likely wrong) in gray
                h = ax.plot(poly[:, 0], poly[:, 1], color="white", linewidth=1.0, alpha=0.35)[0]
                continue

            dx = float(trans[0, col])
            dy = float(trans[1, col])
            rot = float(trans[2, col])
            poly_w = _apply_dxdyrot_to_points(poly, dx=dx, dy=dy, rot_deg=rot)

            h = ax.plot(poly_w[:, 0], poly_w[:, 1], color=cmap(j), linewidth=2.0)[0]
...
def launch(mouse_folder: Optional[str] = None, mouse_table: Optional[str] = None):
    ...
    app = CellRegViewer(t, mouse_folder=mf)
    app.mainloop()
