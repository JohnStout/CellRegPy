...
class CellRegViewer(tk.Tk):
    ...
    def _log(self, msg: str):
        ...
        Always prints to console; if debug_txt exists, appends there too.
        Buffers messages until widgets are built.
        """
        import time
        line = f"[{time.strftime('%H:%M:%S')}] {msg}"
        # Always print for external visibility
        try:
            print(line)
        except Exception:
            pass
        # Buffer until debug box exists
        try:
            self._log_buffer.append(line)
        except Exception:
            pass
        # If debug box exists, flush buffer
        if hasattr(self, "debug_txt"):
            try:
                # flush only once per call
