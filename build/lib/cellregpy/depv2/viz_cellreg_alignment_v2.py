"""
viz_cellreg_alignment_v2.py

Python GUI to validate CellRegPy registration using mouse_table outputs.

Important naming note:
- "C" here means a calcium trace array (often "deconvolved/denoised calcium", CNMF-style),
  NOT the C or C++ programming language. This matches the MATLAB viewer logic:
  prefer o.C if present, otherwise fall back to o.F.  (See vizCellRegAlignment.m)
- "S" here means a spike / deconvolution trace (or Suite2p spks).

What it does:
- Session dropdown
- Background (ops.max_proj or meanImgE/meanImg) + ROI outlines (stat.xpix/ypix)
- Click an ROI outline:
  - open a window showing that CellRegID across sessions (outlines over bg)
  - plot traces across sessions (F / dF/F / C (deconv)) + optional Fneu and spikes (S/spks)

Inputs:
- mouse_table.pkl or mouse_table.csv generated by your CellRegPy pipeline
  required columns: Session, suite2pID, cellRegID, SessionPath
- per-session Suite2p outputs reachable from SessionPath:
  expects .../suite2p/plane0/Fall.mat somewhere under that folder

Run:
  python viz_cellreg_alignment_v2.py --mouse-folder "C:\path\to\mouse_folder"
  python viz_cellreg_alignment_v2.py --mouse-table  "C:\path\to\mouse_table.pkl"
"""
...
def _ops_to_dict(ops: Any) -> Dict[str, Any]:
    ...
    if isinstance(ops, dict):
        return ops
    out: Dict[str, Any] = {}
    for k in ("meanImgE", "meanImg", "max_proj", "Ly", "Lx", "fs", "nframes", "xoff", "yoff"):
        try:
            if hasattr(ops, k):
                out[k] = getattr(ops, k)
        except Exception:
...
class SessionData:
    ...
    C: Optional[np.ndarray] = None   # "C" trace (deconv/denoised calcium), not C/C++
...
class SessionCache:
    ...
    def _load_session(self, session: str, session_path: Path) -> SessionData:
        ...
        # Load full Fall.mat so we can access optional keys (C/S) if present.
        ...
        C = _as_array(data.get("C", None))   # optional
        S = _as_array(data.get("S", None))   # optional
...
class CellRegViewer(tk.Tk):
    def __init__(self, mouse_table: pd.DataFrame):
        ...
        self.mouse_name = str(pd.unique(self.t["MouseName"])[0]) if "MouseName" in self.t.columns else "mouse"
        ...
        # Trace choices: keep explicit naming to avoid "C means C++" confusion.
    ...
    def _show_help(self):
        msg = """Workflow:
    1) Pick a session.
    2) Click a ROI outline.
    3) A window pops up showing that CellRegID across sessions.
    4) Bottom panel shows traces.

    Trace naming:
    - F (Suite2p): raw fluorescence
    - dF/F (from F): baseline-normalized view
    - C (deconv calcium): optional C trace if present in Fall.mat; otherwise viewer falls back to F.
    - Spikes: uses S if present, otherwise spks/s2pSpk.
    """
