...
class CellRegConfig:
    ...
    alignment_fallback_mode: str = 'mean_then_footprints'
...
class CellRegPy:
    ...
    def _run_single_mouse(self, mouse_folder: Path):
        ...
        # Optional fallback images built from spatial footprints
        footprint_images = None
        fallback_mode = str(getattr(self.config, 'alignment_fallback_mode', 'mean_then_footprints')).strip().lower()
        if fallback_mode in ('mean_then_footprints', 'footprints'):
            print("Building spatial-footprint projection images for fallback alignment...")
            fp_arrays = []
            for sess in sess_fovs:
    ...
    def _align_with_optional_fallback(self,
                                      ...
                                      *,
                                      ...
                                      plot_fig: bool = False):
        """
        Try mean-image alignment first, then optionally fall back to footprint projection alignment.

        Returns:
            registered, method, peak, tform, filter_name, outliers, source
        where source is one of {'mean_image', 'spatial_footprints'}.
        """
        cfg = self.config
        fallback_mode = str(getattr(cfg, 'alignment_fallback_mode', 'mean_then_footprints')).strip().lower()

        use_mean = fallback_mode in ('mean_then_footprints', 'none')
        use_footprints = fallback_mode in ('mean_then_footprints', 'footprints')

        if use_mean:
            reg, method, peak, tform, filt, outliers = self.aligner.align(
                fixed_img,
                moving_img,
            ...
            if (not self._alignment_needs_fallback(peak)) or (not use_footprints):
                return reg, method, peak, tform, filt, outliers, 'mean_image'
            print("  Mean-image alignment below threshold; trying spatial-footprint fallback...")

        if use_footprints:
            if fixed_fp_img is None or moving_fp_img is None:
                if use_mean:
                    return reg, method, peak, tform, filt, outliers, 'mean_image'
                raise ValueError("Footprint fallback requested but footprint projection images were not provided.")

            reg2, method2, peak2, tform2, filt2, outliers2 = self.aligner.align(
                fixed_fp_img,
                moving_fp_img,
            ...
            return reg2, method2, peak2, tform2, filt2, outliers2, 'spatial_footprints'

        # Should only hit when fallback_mode == 'none' but mean path disabled somehow
        return reg, method, peak, tform, filt, outliers, 'mean_image'
