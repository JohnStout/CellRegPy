...
class CellRegViewer(tk.Tk):
    def __init__(self):
        ...
        self.session_root_map: Dict[str, Path] = {}
    ...
    def _on_table_select(self, _evt=None):
        # This handler fires both for user clicks and programmatic selection_set().
        # We must suppress the programmatic one to avoid infinite loops.
        if self.t is None:
            return
        if self._suppress_table_event:
            # consume exactly one queued programmatic event
            self._suppress_table_event = False
            return
        ...
        # If we're already on this row, ignore (prevents click -> selection_set loop edge cases)
        if getattr(self, "_selected_row", None) == row_idx:
            return

        self._log(f"[ui] table select row={row_idx}")
        self._select_row(row_idx, open_popups=bool(self.enable_popups.get()))
    ...
    def set_table(self, mouse_table: pd.DataFrame):
        ...
        # Map session label -> session root (SessionPath)
        self.session_root_map = {}
        for s in self.sessions:
            rr = t.index[t["Session"] == s].to_numpy()
            if rr.size:
                self.session_root_map[str(s)] = Path(str(t.loc[rr[0], "SessionPath"]))
    ...
    def _on_pick(self, event):
        ...
        if row_idx is None:
            return
        self._log(f"[ui] pick row={row_idx}")
        self._select_row(int(row_idx), open_popups=bool(self.enable_popups.get()))
    ...
    def _select_row(self, row_idx: int, open_popups: bool = True):
        ...
        try:
            ...
            cid_raw = self.t.loc[row_idx, "cellRegID"]
            cid_num = pd.to_numeric(cid_raw, errors="coerce")
            cid = int(cid_num) if np.isfinite(cid_num) else int(cid_raw)  # fallback

            suite2p_sel_raw = self.t.loc[row_idx, "suite2pID"]
            suite2p_num = pd.to_numeric(suite2p_sel_raw, errors="coerce")
            suite2p_sel = int(suite2p_num) if np.isfinite(suite2p_num) else int(suite2p_sel_raw)

            fov_val = _fov_scalar(self.t.loc[row_idx, "fovID"]) if "fovID" in self.t.columns else None
            self._log(f"[ui] row={row_idx} cellRegID={cid} (raw={cid_raw}) suite2pID={suite2p_sel} fovID={fov_val}")

            # siblings
            t1 = time.perf_counter()
            cid_series = pd.to_numeric(self.t["cellRegID"], errors="coerce")
            if fov_val is None:
                sib_mask = (cid_series == cid)
            else:
                fov_series = pd.to_numeric(self.t["fovID"], errors="coerce")
                sib_mask = (cid_series == cid) & (fov_series == float(fov_val))
            siblings = self.t.index[sib_mask.fillna(False)].to_numpy()
            if siblings.size == 0:
                # Ensure at least self row is included (prevents weird empty selection states)
                siblings = np.array([row_idx], dtype=int)
                self._log("[ui] WARNING: siblings mask empty; using self row only")
            ...
            # programmatic selection: set suppress flag and let _on_table_select consume it
            if str(int(row_idx)) in self.tree.get_children():
                self._suppress_table_event = True
                self.tree.selection_set(str(int(row_idx)))
                self.tree.see(str(int(row_idx)))
                # failsafe: clear in case no event is generated (e.g., selection unchanged)
                self.after(200, lambda: setattr(self, '_suppress_table_event', False))
            ...
            if open_popups and bool(self.enable_popups.get()) and siblings.size > 0:
                ...
                # Build one panel per session (even if missing in some sessions)
                items = []
                cid_series_all = pd.to_numeric(self.t["cellRegID"], errors="coerce")
                fov_series_all = pd.to_numeric(self.t["fovID"], errors="coerce") if "fovID" in self.t.columns else None
                for sess_label in self.sessions:
                    mask0 = (self.t["Session"] == sess_label) & (cid_series_all == cid)
                    cand = self.t.index[mask0.fillna(False)].to_numpy()
                    if cand.size and (fov_val is not None) and (fov_series_all is not None):
                        mask1 = mask0 & (fov_series_all == float(fov_val))
                        cand2 = self.t.index[mask1.fillna(False)].to_numpy()
                        if cand2.size:
                            cand = cand2
                    row_for_session = int(cand[0]) if cand.size else None
                    items.append((str(sess_label), row_for_session))
                items_copy = list(items)
                self.after(1, lambda it=items_copy, c=cid: self._show_footprints(it, c))
    ...
    def _show_footprints(self, items: List[Tuple[str, Optional[int]]], cid: int):
        """
        Render one subplot per session in a Tk Toplevel window without blocking the main GUI.

        items: [(session_label, row_index_or_None), ...] in chronological session order.
               If row is None, we still show the background and label "missing".
        """
        assert self.t is not None
        n = int(len(items))
        if n <= 0:
        ...
        self._log(f"[render] show_footprints: panels={n} (cid={cid})")
        ...
        # queue: (ax, session_label, row_index_or_None)
        self._fp_queue = [(axes[i], items[i][0], items[i][1]) for i in range(n)]
    ...
    def _render_fp_next(self, cid: int):
        ...
        ax, sess, r = self._fp_queue.pop(0)

        try:
            sess_root = self.session_root_map.get(str(sess), None)
            if sess_root is None:
                ax.text(0.5, 0.5, f"No SessionPath for\n{sess}", ha='center', va='center', transform=ax.transAxes)
                self._fp_canvas.draw_idle()
                self._fp_job = self.after(1, lambda: self._render_fp_next(cid))
                return
            ...
            bg = np.asarray(sd.bg, dtype=float)
            if r is None:
                _imshow_gray(ax, bg, title=f"{_truncate(sess, 22)} | MISSING\n{sd.bg_transform}")
                ax.text(0.5, 0.08, "No row in mouse_table for this CellRegID", ha="center", va="center",
                        fontsize=8, transform=ax.transAxes, color="white",
                        bbox=dict(boxstyle="round,pad=0.2", facecolor="black", alpha=0.35))
                ax.set_aspect("equal")
            else:
                roi_1b = int(self.t.loc[int(r), "suite2pID"])
                rid0 = roi_1b - 1  # mouse_table suite2pID is 1-based
                ...
                if 0 <= rid0 < len(sd.stat):
                    ...
                    ax.text(0.5, 0.5, f"suite2pID out of range\nrid0={rid0}", ha='center', va='center', transform=ax.transAxes)
