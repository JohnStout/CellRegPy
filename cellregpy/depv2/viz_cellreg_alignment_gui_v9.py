...
class SessionCache:
    ...
    def get(self, session: str, session_root: Path) -> SessionData:
    ...
    def _load_session(self, session: str, session_root: Path) -> SessionData:
        ...
        # Prefer npy stat/ops (canonical Suite2p orientation)
        ops: Dict[str, Any]
        stat_list: List[Any]
        iscell_b: np.ndarray
        traces: Dict[str, np.ndarray] = {}
        ...
        if fm is not None and used == "npy":
            try:
...
class CellRegViewer(tk.Tk):
    ...
    def _draw_session_image_and_rois(self):
        ...
        sd = self.cache.get(self.current_session, session_root)
    ...
    def _select_row(self, row_idx: int, open_popups: bool = True):
        ...
        self.debug_txt.delete("1.0", tk.END)
        self.debug_txt.insert(tk.END, f"Selected row={row_idx}\ncellRegID={cid} | suite2pID(1-based)={suite2p_sel} | fovID={fov_val}\n\n")
        for r in siblings:
            self.debug_txt.insert(tk.END, f"row={int(r)} | session={self.t.loc[r,'Session']} | suite2pID={int(self.t.loc[r,'suite2pID'])}\n")

        self.status.configure(text=f"Selected cellRegID={cid} | suite2pID={suite2p_sel} | siblings={siblings.size}")

        if open_popups and siblings.size > 0:
            # Schedule popup after returning to Tk event loop (avoids UI freeze on Windows)
            sib_copy = np.array(siblings, copy=True)
            self.configure(cursor='watch')
            self.status.configure(text=f"Rendering footprints for CellRegID {cid}...")
            self.update_idletasks()
            self.after(1, lambda r=sib_copy, c=cid: self._popup_native_footprints(r, c))
            self.after(10, lambda: self.configure(cursor=''))
    ...
    def _popup_native_footprints(self, rows: np.ndarray, cid: int):
        ...
        try:
            ...
            fig, axes = plt.subplots(
                nrows, cols,
                figsize=(min(16, 4 * cols), min(12, 3.8 * nrows))
            )
            ...
            for j, r in enumerate(rows):
                ...
                rid0 = roi_1b - 1  # NOTE: keep this consistent with your mouse_table convention
            ...
            fig.suptitle(
                f"Spatial footprints (native coords) | CellRegID {cid}",
                fontsize=12,
                fontweight="bold"
            )
            ...
            traceback.print_exc()
            try:
                messagebox.showerror("Footprint popup error", str(e))
            except Exception:
                pass

        finally:
            # if you set cursor='watch' before calling, make sure it clears even on error
            try:
