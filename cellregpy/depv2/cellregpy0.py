...
class CellRegConfig:
    ...
    model_type: str = "auto"
    auto_simple_peak_threshold: float = 0.97    
...
class CellRegPy:
    ...
    def _register_fov(self,
        ...
        # Auto-switch to deterministic registration when sessions are already extremely similar.
        # This avoids degenerate mixture-model fits (a known CellReg failure mode).
        use_simple_mode = False
        if bool(getattr(cfg, 'auto_simple_on_high_similarity', False)):
            ...
            if np.isfinite(raw_med) and np.isfinite(ali_med) and (raw_med >= raw_thr) and (ali_med >= ali_thr):
                peak_med = np.nanmedian(maximal_cross_correlation[mask]) if mask.any() else float("nan")
                peak_thr = float(getattr(cfg, "auto_simple_peak_threshold", 0.97))

                if (
                    (np.isfinite(raw_med) and np.isfinite(ali_med) and raw_med >= raw_thr and ali_med >= ali_thr)
                    or
                    (np.isfinite(peak_med) and peak_med >= peak_thr)
                ):
                    use_simple_mode = True
                    print(
                        f"  ⚡ High similarity detected "
                        f"(median raw corr={raw_med:.3f}, median aligned corr={ali_med:.3f}, median peak={peak_med:.3f})."
                    )                
                use_simple_mode = True

        if use_simple_mode and str(getattr(cfg, 'auto_simple_method', 'iou_hungarian')).strip().lower().startswith('iou'):
            print("  Switching to SIMPLE IoU+Hungarian registration (skipping probabilistic CellReg modeling).")
            max_dist_px = float(getattr(cfg, 'simple_dist_threshold_um', 6.0)) / float(cfg.microns_per_pixel)
            cell_to_index_map, centroid_distance_map, spatial_correlation_map, registered_distances, non_registered_distances = (
                initial_registration_iou_hungarian(
                    aligned_fps,
                    aligned_centroid_locations,
                    reference_session_index=ref_idx,
                    maximal_distance=max_dist_px,
                    mask_threshold=float(getattr(cfg, 'simple_mask_threshold', 0.20)),
                    iou_threshold=float(getattr(cfg, 'simple_iou_threshold', 0.25)),
                    cost_beta=float(getattr(cfg, 'simple_cost_beta', 0.25)),
                )
            )
            model_used = 'Simple IoU+Hungarian'
            ...
            p_same_models = {}
            # Save minimal results and return early
            results = {
            ...
            try:
                savemat(str(results_dir / 'cell_to_index_map.mat'), {'cell_to_index_map': cell_to_index_map}, do_compression=True)
            except Exception as e:
